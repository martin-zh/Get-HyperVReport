...
<#
	.SYNOPSIS
		...
		Version History:
 			[x] Version 2.0 - 09.November.2017 - Martin Hess: some fixes for modern browsers, added DeleteOldReport and ClearLog Parameters
			[ ] Version 1.9 - 09.November.2017 - RHE: Change Date & Time Format
	...
	.PARAMETER DeleteOldReport
	
		Removes old report file. Default is $false.
	...
	.PARAMETER ClearLog
		...
		Delete the log file so that it doesn't keep getting bigger and bigger. The default value is "$false".
...
Param (
	...
	[parameter(
                Mandatory=$false,
                HelpMessage='Deletes old HTML reports with the same prefix in the destination folder ($true/$false). The default is $false')]
                [bool]$DeleteOldReport = $false,
	...
	[parameter(
                Mandatory=$false,
                HelpMessage='Clears the log file before starting a new run ($true/$false). The default is $false')]
                [bool]$ClearLog = $false
)
    ...
    # Print MSG
    sPrint -Type 1 -Message "Started! Hyper-V Reporting Script (Version 2.0)"
    ...
    if ($DeleteOldReport) {
        # Wir suchen nach Dateien, die dem genauen Namensmuster der Reports entsprechen
        $SearchPattern = "$Cluster-$ReportFileNamePrefix*.html"
        
        # Holen aller passenden Dateien, schließen aber den potenziell gerade neu definierten Pfad aus
        $OldReports = Get-ChildItem -Path $ReportFilePath -Filter $SearchPattern -File -ErrorAction SilentlyContinue | Where-Object { $_.FullName -ne $ReportFile }
        
        if ($OldReports) {
            # Wir nutzen die script-eigene sPrint Funktion für sauberes Logging
            sPrint -Type 1 -Message "Deleting $($OldReports.Count) old report file(s)..." -WriteToLogFile $True
    ...
    if ($ClearLog -and (Test-Path -Path $LogFile)) {
    ...
    #ReportBody{
		width:95%;
		height:auto; /* 'auto' oder '500px' verwenden */
		/*border: 1px solid;*/
		margin: 0 auto;
	}
